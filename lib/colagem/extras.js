...
(function (G) {
  ...
  // ---------- marca-d'água (logo CP2B no canto) ----------
  /** Logo colorido num cartão branco limpo (manual da marca: sem sombra, textura ou contorno sobre o logo).
   *  o: w (largura do logo, px), margin, y, t0/t1 (entra/sai, s), name (imagem, padrão 'logo'). */
  function watermark(ctx, W, H, t, o = {}) {
    const logo = images.get(o.name ?? 'logo');
    if (!logo) return;
    const a = clamp((t - (o.t0 ?? 0.6)) / 0.5) * (1 - clamp((t - (o.t1 ?? 1e9)) / 0.35));
    if (a <= 0) return;
    const lw = o.w ?? 190, lh = lw * logo.height / logo.width, px = lw * 0.075, py = lw * 0.06;
    const cw = lw + 2 * px, ch = lh + 2 * py;
    const x = W - (o.margin ?? 34) - cw, y = o.y ?? (o.margin ?? 34);
    ctx.save();
    ctx.setTransform(1, 0, 0, 1, 0, 0);
    ctx.globalAlpha = a;
    ctx.fillStyle = '#ffffff';
    ctx.beginPath(); ctx.roundRect(x, y, cw, ch, ch * 0.16); ctx.fill();
    ctx.drawImage(logo, x + px, y + py, lw, lh);
    ctx.restore();
  }

  C.extras = { watermark, loadImages, img, imgSize, images, microbe, atom, molecule, bubble, flame, gearPiece, pipe, pointAt, tag, confetti, fly, pie, lens };
})(window);
